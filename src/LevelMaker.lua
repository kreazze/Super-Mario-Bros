...
function LevelMaker.generate(width, height)
    ...
    for x = 1, width do
        ...
        if math.random(7) == 1 then
            ...
            -- spawn a key block
            if math.random(10) == 1 and not spawnKey then
                spawnKey = true
                table.insert(objects,
                    GameObject {
            ...
            -- spawn a lock block
            if math.random(5) == 1 and not spawnLock then
                spawnLock = true
                table.insert(objects,
                    GameObject {
                        texture = 'keys-and-locks',
                            ...
                            if player.hasKey then 
                                ...
                                local flag = GameObject {
                                    ...
                                    onCollide = function(player, obj)
                                        obj.hit = true
                                        obj.solid = false
                                    -- make the flag move up from the post and play a sound
                                    Timer.tween(0.5, {
                                            [obj] = {y = 3 * TILE_SIZE}
                                        })                                        
                                        gSounds['powerup-reveal']:play()
    ...
    if not spawnLock or not spawnKey then 
        gStateMachine:change('play')
    end
